...
class PSELoss(nn.Module):
    ...
    def single_sample_loss(self, pred, label, training_mask):
        ...
        # M = self.cal_M(pred_n, (pred_n >= 0.5).float())
        M = self.cal_M1(pred_n, pred_n)
    ...
    def cal_M(self, pred_n, label_n):
        ...
        M = label_n.clone()  # h*w
        # 计算文本区域像素点个数
        nonzero_sum = label_n.sum().int()
        # 拿到背景区域的x,y索引
        zero_index = (label_n == 0).nonzero()
        zero_index = zero_index[:, 0]

        # 从预测图里拿到背景像素的分数
        zero_predict_score = pred_n[zero_index]
        # 按照OHEM的比例选取背景像素的x,y索引
        select_zero_index = zero_predict_score.argsort()[-nonzero_sum * self.ratio:]
        select_index = zero_index[select_zero_index]
        # 将相应的背景像素设为选择
        M[select_index] = 1
        return M

    def cal_M1(self, pred_n, label_n):
        """
        使用OHEM算法选择参与计算loss的文本和背景像素矩阵,单个样本
        :param pred_n: 完整文本实例的预测结果
        :param label_n: 完整文本实例的ground truth
